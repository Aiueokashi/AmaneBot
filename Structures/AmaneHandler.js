...
const { AmaneHandlerEvents } = require('./Command/Constants');
const AmaneModule = require('./AmaneModule');
const Category = require('./Extender/Category');
...
class AmaneHandler extends EventEmitter {
    ...
    load(thing, isReload = false) {
        ...
        if (this.modules.has(mod.id)) throw new AmaneError('ALREADY_LOADED', this.classToHandle.name, mod.id);
    ...
    remove(id) {
        ...
        if (!mod) throw new AmaneError('MODULE_NOT_FOUND', this.classToHandle.name, id);
    ...
    reload(id) {
        const mod = this.modules.get(id.toString());
        if (!mod) throw new AmaneError('MODULE_NOT_FOUND', this.classToHandle.name, id);
        if (!mod.filepath) throw new AmaneError('NOT_RELOADABLE', this.classToHandle.name, id);
