...
module.exports = class ReactionHandler extends ReactionCollector {
  constructor(message, filter, options, display, emojis) {
    ...
    this.methodMap = new Map(
      Object.entries(this.display.emojis).map(([key, value]) => [value, key])
    );
    this.currentPage = this.options.startPage || 0;
    this.prompt = this.options.prompt || "番号を送信してください。";
    this.time =
      typeof this.options.time === "number" ? this.options.time : 30 * 1000;
    this.awaiting = false;
    ...
    this.on("collect", (reaction, handler) => {
      ...
      message.reactions
        .resolve(reaction)
        .users.remove(reaction.users.cache.last());
      this[this.methodMap.get(reaction.emoji.id || reaction.emoji.name)](
        handler,
