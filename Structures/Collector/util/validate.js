...
module.exports.validateOptions = (options, type) => {
  if (!options)
    return Promise.reject(
      new Error("Missing arguments: options is undefined.")
    );
  if (!options.botMessage || !options.botMessage.client)
    return Promise.reject(
      new Error("Invalid input: botMessage is undefined or invalid.")
    );
  ...
  if (typeof client.users.resolve === "function")
    validOptions.user = client.users.resolve(options.user);
  else validOptions.user = client.resolver.resolveUser(options.user);
  if (!options.user)
    return Promise.reject(
      new Error("Invalid input: user is undefined or invalid.")
    );

  if (options.botMessage.channel.type === "text") {
    if (!options.botMessage.guild)
      return Promise.reject(
        new Error("Invalid input: botMessage.guild is undefined.")
      );
    ...
    if (options.botMessage.guild.me) {
      if (
        ...
        return Promise.reject(
          new Error(
            "Missing permissions: I cannot react in messages in that channel."
          )
        );
    }
  }
  ...
  if (options.botMessage.channel.type === "text") {
    ...
    if (
      ...
      return Promise.reject(
        new Error(
          "Missing permissions: I not have permissions to Manage Messages in this channel to delete reactions."
        )
      );
  ...
  switch (type) {
    ...
    case "reactMenu":
      ...
      if (
        ...
        return Promise.reject(
          new Error("Invalid input: reactions is invalid type.")
        );

      if (!options.reactions) {
        validOptions.reactionsMap =
          type !== "reactPaginator"
    ...
    case "messageAsyncQuestion":
      if (options.onMessage && typeof options.onMessage !== "function")
        return Promise.reject(
          new Error("Invalid input: onMessage is invalid type.")
        );

      if (!options.onMessage)
        validOptions.onMessage = Constants.DEFAULT_RETURN_FUNCTION;

      if (options.botMessage.channel.type === "dm")
        validOptions.deleteMessage = false;
      else if (!isBoolean(options.deleteMessage))
        validOptions.deleteMessage = Boolean(options.deleteMessage);
      ...
      if (
        ...
        return Promise.reject(
          new Error(
            "Missing permissions: I not have permissions to Manage Messages in this channel to delete messages."
          )
        );
    ...
    default:
      return Promise.reject(
        new Error(`Invalid type: '${type}' is not a valid type.`)
      );
  }
  ...
  if (type === "reactMenu" || type === "reactPaginator") {
    if (!options.pages)
      return Promise.reject(
        new Error(
          "Invalid input: You need add pages to create a react menu/paginator."
        )
      );
    ...
    } else if (!isArray(options.pages))
      return Promise.reject(
        new Error(
          "Invalid input: Pages of react paginator must be array of MessageEmbed"
        )
      );
  ...
  if (isNaN(options.collectorOptions.time)) {
    validOptions.collectorOptions.time = parseInt(
      options.collectorOptions.time
    );
    if (isNaN(options.collectorOptions.time)) {
  ...
  if (isNaN(options.collectorOptions.max)) {
    ...
    if (isNaN(options.collectorOptions.max)) {
      if (
        ...
        validOptions.collectorOptions.max =
          Constants.DEFAULT_PAGINATOR_MAX_REACT;
      } else {
        validOptions.collectorOptions.max =
          Constants.DEFAULT_COLLECTOR_MAX_REACT;
