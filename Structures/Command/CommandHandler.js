...
const { deepAssign, flatMap, intoArray, intoCallable, isPromise, prefixCompare } = require('../Utils/Util');
...
class CommandHandler extends AmaneHandler {
    constructor(client, {
        ...
        if (!(classToHandle.prototype instanceof Command || classToHandle === Command)) {
            throw new AmaneError('INVALID_CLASS_TO_HANDLE', classToHandle.name, Command.name);
        }
    ...
    register(command, filepath) {
        ...
        for (let alias of command.aliases) {
            ...
            if (conflict) throw new AmaneError('ALIAS_CONFLICT', alias, command.id, conflict);
            ...
            if (this.aliasReplacement) {
                ...
                if (replacement !== alias) {
                    ...
                    if (replacementConflict) throw new AmaneError('ALIAS_CONFLICT', replacement, command.id, replacementConflict);
